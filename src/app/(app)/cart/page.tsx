...
const Cart = () => {
    ...
    const subtotal = cartItems.reduce((acc, item) => acc + item.price * quantity, 0);
                ...
                <div className="space-y-4 md:space-y-6">
                    {
                        cartItems.map((item: TCart) =>
                            <div key={item.id} className="pb-4 md:pb-6 last:border-b-0">
                                <div className="grid grid-cols-1 md:grid-cols-12 gap-4 md:gap-4 items-center">
                                    <div className="md:col-span-5 flex gap-3 md:gap-4">
                                        ...
                                        <div className="font-medium text-sm md:text-base space-y-1">
                                            ...
                                            <h3 className="text-gray-600">Material: {item.material}</h3>
                                            <h3 className="text-gray-600">Color: {item.color}</h3>
                                        </div>
                                    </div>
                                    ...
                                    <div className="md:col-span-2 flex justify-between md:justify-center items-center">
                                        ...
                                        <div className="border border-gray-400 rounded-lg flex items-center gap-2 px-2 py-1 md:py-2">
                                            <button onClick={hanldeAddQuantity} className="cursor-pointer hover:text-blue-500 transition">
                                                <IoAdd size={16} className="md:w-5 md:h-5" />
                                            </button>
                                            <span className="font-semibold text-sm md:text-base">{quantity}</span>
                                            <button onClick={handleSubtractQuantity} className="cursor-pointer hover:text-blue-500 transition">
                                    ...
                                    <div className="md:col-span-2 flex justify-between md:justify-center items-center">
                                        ...
                                        <div className="text-base md:text-lg font-semibold">${item.price}</div>
                                    </div>
